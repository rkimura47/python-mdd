...
# %% auto 0
__all__ = ['MDDNodeState', 'MDDArcLabel', 'MDDNode', 'MDDArc', 'MDDNodeInfo', 'MDDArcInfo', 'MDD']
...
@dataclass()
class MDDArcInfo:
    """MDDArcInfo represents information associated with an MDDArc.
...
class MDD:
    ...
    arc_info: dict[MDDArc, MDDArcInfo]
        info of MDD arcs (default: dict())

    """
    name: str = "mdd"
    nodes: list[dict[MDDNode, MDDNodeInfo]] = field(default_factory=list)
    arc_info: dict[MDDArc, MDDArcInfo] = field(default_factory=dict)
        ...
        if show_long:
            ...
            for (j, lyr) in enumerate(self.nodes):
                ...
                for v in lyr:
                    ...
                    s += '>\n'
            s += '# (Outgoing) Arcs\n'
            s += '\n'.join(str(a) + ":" + str(self.arc_info[a]) for a in self.all_outgoing_arcs())
            if show_incoming:
                s += '\n# (Incoming) Arcs\n'
                s += '\n'.join(str(a) + ":" + str(self.arc_info[a]) for a in self.all_incoming_arcs())
        else:
            # Short form
            s += '# Nodes\n'
            for (j, lyr) in enumerate(self.nodes):
            ...
            s += '# (Outgoing) Arcs\n'
            s += ', '.join(str(a) + ":" + str(self.arc_info[a]) for a in self.all_outgoing_arcs())
            if show_incoming:
                s += '\n# (Incoming) Arcs\n'
                s += ', '.join(str(a) + ":" + str(self.arc_info[a]) for a in self.all_incoming_arcs())
        return s
    ...
    def add_arc(
        ...
        self.arc_info[newarc] = info

    def add_arcs(
        self,
        newarcs: Mapping[MDDArc, MDDArcInfo], # arcs to be added
    ) -> None:
        """Add arcs to the MDD, without any sanity checks."""
        for arc, info in newarcs.items():
            self.add_arc(arc, info)
    ...
    @staticmethod
    def _default_awfun(info: MDDArcInfo, ns: MDDNodeState, nt: MDDNodeState, j: int) -> MDDArcInfo:
        """By default, use the original arc info."""
        return info
    ...
    def merge_nodes(
        ...
        awinfun: Optional[Callable[[MDDArcInfo, MDDNodeState, MDDNodeState, int], MDDArcInfo]] = None,
        awoutfun: Optional[Callable[[MDDArcInfo, MDDNodeState, MDDNodeState, int], MDDArcInfo]] = None,
    ) -> MDDNode:
        ...
        awinfun : Optional[Callable[[MDDArcInfo, MDDNodeState, MDDNodeState, int], MDDArcInfo]]
            awinfun(w,os,ms,j) returns the adjusted info of an arc with info 'w', old head node state 'os', and new head node (i.e., merged supernode in layer 'j') state 'ms';
            if awfun is None (default), the original info is used
        awoutfun : Optional[Callable[[MDDArcInfo, MDDNodeState, MDDNodeState, int], MDDArcInfo]]
            awoutfun(w,os,ms,j) returns the adjusted info of an arc with info 'w', old tail node state 'os', and new tail node (i.e., merged supernode in layer 'j') state 'ms';
            if awoutfun is None (default), the original info is used
